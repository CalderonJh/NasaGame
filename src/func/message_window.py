...
from src.tools import path, modify
...
def message_top(root: ctk.CTk, pos_x: int, pos_y: int):
    ...
    f_messages = tk.Frame(w_messages,
                          height=240,
                          width=623,
                          bg='#2C3958')

    f_messages.grid(row=1,
                    column=0,
                    rowspan=4)

    f_bottom = tk.Frame(w_messages,
                        height=60,
                        width=623,
                        bg='#2C3958')
    ...
    def delete_selected():
        ...
        for msg_index in seleccionados[::-1]:
            try:
                ...
                msg = modify.get_messages(path_msgs)[msg_index]
                modify.delete_message(msg_index, path_msgs)
                if msg in modify.get_messages(path.resource_path('config-ng\\show_msg.txt')):
                    modify.delete_show(msg)
            except IndexError:
    ...
    def delete_all():
        try:
            modify.delete_all_messages(path_msgs)
            modify.delete_all_messages(path.resource_path('config-ng\\show_msg.txt'))
    ...
    def show():
        ...
        for i in selected:
            modify.add_message(modify.get_messages(path_msgs)[i],
                               path.resource_path('config-ng\\show_msg.txt'))

    # widgets
    ctk.CTkButton(f_top, text='Show',
                  width=60,
                  fg_color='#343638',
                  border_color="White",
                  command=show,
                  border_width=1,
                  text_color='White').grid(row=0,
                                           column=1,
                                           padx=10,
                                           pady=5)
    ...
    msg_option_time = ctk.CTkOptionMenu(f_top,
                                        width=5,
                                        height=20,
                                        fg_color='#343638',
                                        values=['7', '5', '3', '1', '0.5'],
    ...
    msg_option_time.set(modify.get_variable('msg_timer'))

    msg_option_time.grid(row=0,
                         column=5,
                         padx=3,
                         pady=5)
    ...
    for item in modify.get_messages(path.resource_path('config-ng\\messages.txt')):
        listbox.insert(tk.END, ' ' + item)

    # crear mensajes
    entry = ctk.CTkEntry(f_bottom,
                         validate="key",
                         validatecommand=(validation, '%P'),
                         border_color='White',
                         border_width=1,
                         placeholder_text="Write a new message",
                         font=('Calibri', 15),
                         width=420)
    ...
    def on_close():
        ...
        try:
            ...
            pass
        modify.set_variable('msg_timer', msg_option_time.get())

    w_messages.protocol("WM_DELETE_WINDOW", on_close)
    ctk.CTkButton(f_bottom,
                  command=create_message,
                  text='Ok',
                  fg_color='#343638',
                  border_color="White",
                  border_width=1,
                  width=45).pack(anchor='e',
                                 side='right',
                                 padx=10,
                                 pady=5)
