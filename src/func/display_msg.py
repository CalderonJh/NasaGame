...
path_show_msg = path.resource_path('config-ng\\show_msg.txt')
...
def show_msg_top(root: ctk.CTk, pos_x: int, pos_y: int):
    ...
    # frames primarios
    f_messages = tk.Frame(window,
                          height=240,
                          width=623,
                          bg='#2C3958')

    f_messages.grid(row=0,
                    column=0,
                    rowspan=4)
    ...
    def selected():
        ...
        for msg_index in seleccionados[::-1]:
            try:
                ...
                modify.delete_message(msg_index, path_show_msg)
            except IndexError:
                print('Error: INDEX ERROR - delete_selected')

    canvas = tk.Canvas(f_messages,
                       bg='darkgray')
    ...
    for item in modify.get_messages(path_show_msg):
